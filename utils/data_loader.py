...
class Block:
    def __init__(self, mat, u_batch_size = 1000, i_batch_size = 1000, device = None): 
        self.mat = mat
        ...
        self.device = device
        self.User_loader = DataLoader(Loader_list(torch.arange(self.n_users).to(self.device)), batch_size=u_batch_size, shuffle=True, num_workers=0)
        self.Item_loader = DataLoader(Loader_list(torch.arange(self.n_items).to(self.device)), batch_size=i_batch_size, shuffle=True, num_workers=0)
    
    def get_batch(self, batch_user, batch_item, device = None): 
...
class User: 
    def __init__(self, mat_position, mat_rating, u_batch_size = 100, device = None):
    ...
    def get_batch(self, batch_user, device = None): 
        # Use instance device if not specified
        if device is None:
            device = self.device
